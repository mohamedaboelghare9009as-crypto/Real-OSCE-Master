...
class SimulationService extends EventEmitter {
    ...
    async initializeCase(caseId?: string, specialty?: string): Promise<Case | null> {
        ...
        try {
            ...
            const data = await res.json();
            // The backend returns { case, session } or just case
            const caseData = data.case || data;
            const newSessionId = data.session?._id || data.sessionId || data.id;

            // Normalize for Frontend Case Type
            const normalizedCase: Case = {
                id: caseData._id || caseData.id || caseId,
                title: caseData.title || caseData.case_metadata?.title || caseData.metadata?.title || "Unknown Case",
                specialty: caseData.specialty || caseData.case_metadata?.specialty || caseData.metadata?.specialty || "General",
                difficulty: caseData.difficulty || caseData.case_metadata?.difficulty || caseData.metadata?.difficulty || "Novice",
                description: caseData.description || caseData.case_metadata?.objective || caseData.metadata?.description || "",
                chiefComplaint: caseData.chiefComplaint || caseData.truth?.history?.chief_complaint || caseData.history?.chiefComplaint || "",
                vitals: caseData.vitals || caseData.truth?.physical_exam?.vitals || caseData.examination?.vitals,
                patientAvatar: caseData.patientAvatar || caseData.case_metadata?.patient_avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=Patient",
                tags: caseData.tags || caseData.case_metadata?.learning_objectives || caseData.metadata?.tags || [],
                systemInstruction: caseData.systemInstruction || caseData.scenario?.candidate_instructions || "",
                fullData: caseData, // Keep full original data for detail views
                patientName: caseData.truth?.demographics?.name || caseData.demographics?.name || "Patient"
            };

            this.currentCaseId = normalizedCase.id;
            this.sessionId = newSessionId;
            (normalizedCase as any).sessionId = newSessionId; // Ensure available for hook
            this.currentCaseData = normalizedCase;

            console.log(`[SimulationService] Initialized: Case=${this.currentCaseId}, Session=${this.sessionId}`);
    ...
    async sendMessage(message: string, history: any[]): Promise<any> {
        ...
        console.log(`[SimulationService] Sending message for Session: ${this.sessionId}, Case: ${this.currentCaseId}`);
