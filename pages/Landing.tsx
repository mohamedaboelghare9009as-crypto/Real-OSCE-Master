        ...
        <div className="min-h-screen bg-[#F6F8FA] overflow-x-hidden selection:bg-emerald-500/30 font-inter">
            ...
            <nav className="relative z-50 max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/auth')} className="text-slate-600 font-bold text-sm hover:text-emerald-600 hidden sm:block">Log In</button>
                    <button
            ...
            {/* Pricing Section */}
            <section id="pricing" className="py-24 relative z-10 bg-[#F6F8FA]">
                <div className="max-w-7xl mx-auto px-6">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-5xl font-bold text-slate-900 mb-4">Simple, Transparent Pricing</h2>
                        <p className="text-lg text-slate-500 max-w-2xl mx-auto">Choose the plan that fits your study needs.</p>
                    </div>

                    <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                        {/* Free Plan */}
                        <div className="bg-white rounded-3xl p-8 border border-slate-200 hover:shadow-xl transition-all duration-300 relative overflow-hidden group">
                            ...
                            <p className="text-slate-500 mb-8 h-12">Perfect for trying out the platform and practicing a few cases.</p>

                            <ul className="space-y-4 mb-8">
                                <li className="flex items-center gap-3 text-slate-700">
                                    <ShieldCheck className="w-5 h-5 text-emerald-500" />
                                    <span>3 AI Case Simulations / mo</span>
                                </li>
                                <li className="flex items-center gap-3 text-slate-700">
                                    <ShieldCheck className="w-5 h-5 text-emerald-500" />
                                ...
                                <li className="flex items-center gap-3 text-slate-700">
                                    <ShieldCheck className="w-5 h-5 text-emerald-500" />
                                    <span>Core Specialties Only</span>
                                </li>
                            </ul>

                            <button
                                onClick={() => navigate('/auth')}
                                className="w-full h-12 rounded-xl border border-slate-300 text-slate-700 font-bold hover:bg-slate-50 transition-colors"
                            >
                                Get Started
                            </button>
                        </div>
                        ...
                        {/* Pro Plan */}
                        <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800 shadow-2xl relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
                            {/* Glow Effect */}
                            <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl -mr-16 -mt-16 group-hover:bg-emerald-500/20 transition-all duration-500"></div>

                            <div className="absolute top-6 right-6 px-3 py-1 bg-emerald-500/20 text-emerald-400 text-xs font-bold rounded-full border border-emerald-500/20 uppercase tracking-wide">
                                Most Popular
                            </div>

                            <h3 className="text-2xl font-bold text-white mb-2 relative z-10">Pro Master</h3>
                            <div className="flex items-baseline gap-1 mb-6 relative z-10">
                                <span className="text-4xl font-bold text-white">$19.99</span>
                                <span className="text-slate-400">/month</span>
                            </div>
                            <p className="text-slate-400 mb-8 h-12 relative z-10">Unlimited access to everything. Ideal for heavy exam prep.</p>

                            <ul className="space-y-4 mb-8 relative z-10">
                                <li className="flex items-center gap-3 text-slate-300">
                                    <div className="p-1 rounded-full bg-emerald-500/20 text-emerald-400"><ShieldCheck className="w-4 h-4" /></div>
                                    <span className="text-white">Unlimited AI Case Simulations</span>
                                </li>
                                <li className="flex items-center gap-3 text-slate-300">
                                    <div className="p-1 rounded-full bg-emerald-500/20 text-emerald-400"><ShieldCheck className="w-4 h-4" /></div>
                                    <span className="text-white">Detailed Competency Scoring</span>
                                </li>
                                <li className="flex items-center gap-3 text-slate-300">
                                    <div className="p-1 rounded-full bg-emerald-500/20 text-emerald-400"><ShieldCheck className="w-4 h-4" /></div>
                                    <span className="text-white">Access to All Specialties</span>
                                </li>
                                <li className="flex items-center gap-3 text-slate-300">
                                    <div className="p-1 rounded-full bg-emerald-500/20 text-emerald-400"><ShieldCheck className="w-4 h-4" /></div>
                                    <span className="text-white">Voice & Text Input</span>
                                </li>
                            </ul>

                            <button
                                onClick={() => navigate('/subscribe')}
                                className="w-full h-12 rounded-xl bg-emerald-500 text-white font-bold shadow-lg shadow-emerald-500/25 hover:bg-emerald-400 transition-all relative z-10"
                            >
                                Upgrade to Pro
                            </button>
                        </div>
