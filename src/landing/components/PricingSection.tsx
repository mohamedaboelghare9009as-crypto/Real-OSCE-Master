...
const TIERS: PricingTier[] = [
    { id: 'essentials', name: 'Essentials', description: 'Core simulation tools for early medical students.', monthlyPrice: 49, annualPrice: 29, color: 'bg-osce-light-blue/300', icon: Stethoscope, features: ['Access to 50+ Core Cases', 'Basic Performance Scoring', 'Standard AI Patient Response', 'Mobile App Access'] },
        ...
        <div style={{ perspective: 1000 }} className="h-full">
            <motion.div ref={ref} onMouseMove={handleMouseMove} onMouseLeave={() => { x.set(0); y.set(0); }} style={{ rotateX, rotateY, transformStyle: "preserve-3d" }} className={`relative h-full flex flex-col p-8 rounded-3xl border transition-all duration-200 ${tier.highlight ? 'bg-white/80 border-osce-orange/50 shadow-xl shadow-osce-orange/10' : 'bg-white/40 border-white/60 shadow-lg'} backdrop-blur-xl`}>
                ...
                <div className="mb-8 relative z-0">
                    ...
                    <p className="text-osce-navy/70 text-sm mt-2 leading-relaxed h-10">{tier.description}</p>
                </div>
                <div className="mb-8 flex items-baseline gap-1">
                    <AnimatedPrice price={price} />
                    <span className="text-osce-navy/60 font-medium">/mo</span>
                ...
                <div className="flex-grow space-y-4 mb-8">
                    {tier.features.map((feat, i) => (
                        <div key={i} className="flex items-start gap-3 text-sm text-slate-700">
                            <div className={`mt-0.5 min-w-[18px] h-[18px] rounded-full flex items-center justify-center ${tier.highlight ? 'bg-amber-100 text-osce-orange' : 'bg-slate-200 text-osce-navy/60'}`}><Check size={10} strokeWidth={3} /></div>
                            <span>{feat}</span>
                        </div>
                    ))}
                </div>
                <button className={`w-full py-4 rounded-xl font-bold text-sm transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] ${tier.highlight ? 'bg-osce-navy text-white hover:bg-osce-orange shadow-xl' : 'bg-white text-osce-navy border border-osce-light-blue hover:bg-osce-light-blue/30'}`}>Select {tier.name}</button>
            </motion.div>
        </div>
    );
};
        ...
        <section className="relative w-full py-24 md:py-32 bg-osce-blue overflow-hidden">
            <div className="relative z-10 max-w-7xl mx-auto px-6">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <p className="text-lg text-osce-navy/70">Choose the plan that fits your exam timeline. Cancel anytime.</p>
                </div>
                <div className="flex justify-center mb-16 relative z-20">
                    <div className="bg-white/60 p-1 rounded-full flex relative backdrop-blur-md border border-white/80 shadow-sm">
                        ...
                        <button onClick={() => toggleBilling('monthly')} className={`relative px-6 py-2 rounded-full text-sm font-bold transition-colors z-10 ${billing === 'monthly' ? 'text-osce-navy' : 'text-osce-navy/60 hover:text-slate-700'}`}>
                            Monthly
                        ...
                        <button onClick={() => toggleBilling('annual')} className={`relative px-6 py-2 rounded-full text-sm font-bold transition-colors z-10 ${billing === 'annual' ? 'text-osce-navy' : 'text-osce-navy/60 hover:text-slate-700'}`}>
                            Annual (Save 40%)
                ...
                <div className="mt-16 text-center">
                    <p className="text-osce-navy/60 text-sm flex items-center justify-center gap-2"><Check size={16} className="text-osce-orange" /> 14-day money-back guarantee on all annual plans.</p>
                </div>
            </div>
        </section>
    );
