...
interface DDxPanelProps {
    onDDxChange?: (diagnoses: DDxItem[]) => void;
}

const DDxPanel: React.FC<DDxPanelProps> = ({ onDDxChange }) => {
    const [isOpen, setIsOpen] = useState(true);
    const [diagnoses, setDiagnoses] = useState<DDxItem[]>([]);
    const [input, setInput] = useState('');

    // Notify parent when diagnoses change
    React.useEffect(() => {
        onDDxChange?.(diagnoses);
    }, [diagnoses, onDDxChange]);
