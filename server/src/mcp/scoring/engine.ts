...
export class ScoringEngine {
    ...
    static async evaluateAction(
        ...
        const alreadyAwarded = matchedItem && session.actionsTaken.some(
            taken => taken.details?.itemId === matchedItem?.item // Assuming we store item SKU/ID or text
        );

        if (matchedItem && !alreadyAwarded) {
            pointsAwarded = matchedItem.weight;
            if (matchedItem.critical) {
                // If it's a positive critical item, good. 
                // If it was a NEGATIVE weight and critical, that's a Critical Error.
                if (matchedItem.weight < 0) {
                    criticalFlags.push(matchedItem.item); // Flag the error
                }
            }
            ...
            // No match found
            // Could be a neutral action or a distraction
        ...
        await ScoringLogger.logAction({
            ...
            timestamp: new Date(),
            details: matchedItem ? { itemId: matchedItem.item } : undefined
        });
