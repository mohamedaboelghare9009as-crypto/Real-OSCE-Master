export const McpToolsDefinition = [
    ...
    {
        name: "nurse_measure_vitals",
        description: "Nurse measures the patient's vitals (HR, BP, RR, SpO2, Temp).",
        parameters: {
            type: "object",
            properties: {
                reason: { type: "string", description: "Why the vitals are being checked." }
            }
        }
    },
    {
        name: "nurse_perform_action",
        description: "Nurse performs a specific clinical action (e.g., blood sample, IV access).",
        parameters: {
            type: "object",
            properties: {
                action: { type: "string", enum: ["take_blood", "iv_access", "ecg", "administer_oxygen", "nebulizer"] },
                site: { type: "string", description: "Where the action is performed (e.g., left arm)." }
            },
            required: ["action"]
        }
    }
