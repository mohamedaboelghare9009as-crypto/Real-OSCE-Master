...
export class OSCE_MCP {
    ...
    async processUserRequest(userId: string, caseId: string, userMessage: string, sessionId?: string, precomputedIntent?: string): Promise<McpResponse> {
        // 1. Pre-Check (Policy Enforcement)
        const isRequestValid = await this.policyEngine.validateRequest(userId, caseId, userMessage, sessionId);
        if (!isRequestValid) {
            console.warn(`[MCP] Policy Blocked: User: ${userId}, Case: ${caseId}`);
            return { content: "Invalid request or session state.", tool_calls: [{ name: 'deny_request', arguments: { reason: 'Policy violation' } }] };
        }

        // 2. Build Safe Context
        const context = await this.contextBuilder.buildContext(caseId, userId);

        // 3. Resolve Tools (Fast-Path or AI-Path)
        let rawResponse: McpResponse;

        if (precomputedIntent && precomputedIntent !== 'UNKNOWN') {
            console.log(`[MCP] Fast-Path Scoring: ${precomputedIntent}`);
            rawResponse = {
                content: userMessage,
                tool_calls: [{ name: 'reveal_info', arguments: { topic: precomputedIntent } }]
            };
        } else if (caseId === 'mock-case-001') {
        ...
        // 6. TOOL EXECUTION (State Impact)
        if (sessionId && safeResponse.tool_calls && safeResponse.tool_calls.length > 0) {
            for (const tool of safeResponse.tool_calls) {
                await this.executeTool(sessionId, caseId, tool);
            }
        }
    ...
    private async executeTool(sessionId: string, caseId: string, tool: { name: string, arguments: any }) {
        console.log(`[MCP] Executing Tool: ${tool.name}`, tool.arguments);
        const session = await sessionService.getSession(sessionId);
        if (!session) return;

        const caseData = await caseService.getCaseById(caseId) as any;

        switch (tool.name) {
            case 'nurse_measure_vitals':
                const trueVitals = caseData?.truth?.physical_exam?.vitals || caseData?.vitals;
                if (trueVitals) {
                    session.dynamicData = { ...session.dynamicData, vitals: trueVitals };
                    await sessionService.updateSession(sessionId, { dynamicData: session.dynamicData });
                    console.log("[MCP] Vitals updated in session state.");
                }
                break;

            case 'nurse_perform_action':
                // Logic for procedures (e.g. taking blood reveals investigations)
                const action = tool.arguments.action;
                if (action === 'take_blood') {
                    const labs = caseData?.truth?.investigations?.bedside || [];
                    session.dynamicData = {
                        ...session.dynamicData,
                        investigations: [...(session.dynamicData?.investigations || []), ...labs]
                    };
                    await sessionService.updateSession(sessionId, { dynamicData: session.dynamicData });
                }
                // Add procedure to checklist/actions
                session.actionsTaken.push({
                ...
                await sessionService.updateSession(sessionId, { actionsTaken: session.actionsTaken });
                break;

            case 'reveal_info':
                // Scoring already handled in processUserRequest, but we ensure revealedFacts is updated
                if (!session.revealedFacts.includes(tool.arguments.topic)) {
                    session.revealedFacts.push(tool.arguments.topic);
                    await sessionService.updateSession(sessionId, { revealedFacts: session.revealedFacts });
                }
                break;
        }
    }
