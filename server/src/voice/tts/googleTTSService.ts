 ...
 */
export class LegacyGoogleTTSService implements TTSService {
    ...
    private async ensureInitialized() {
        ...
        if (!fs.existsSync(this.serviceAccountPath)) {
            console.warn(`[TTS-Voice] Key not found at ${this.serviceAccountPath}. Falling back to mock.`);
            return;
        }
        ...
        try {
            ...
            console.log('[TTS-Voice] Google Cloud TTS Ready (Lazy)');
        } catch (e) {
            console.error("[TTS-Voice] Failed to init:", e);
        }
    }
...
export const ttsService = new LegacyGoogleTTSService();
