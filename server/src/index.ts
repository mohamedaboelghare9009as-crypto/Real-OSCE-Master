...
// --- LOCK FILE MECHANISM REMOVED FOR DEV FLOW ---
/*
const LOCK_FILE = path.resolve(__dirname, '../server.lock');

if (fs.existsSync(LOCK_FILE)) {
    console.error(`[FATAL] Lock file exists at ${LOCK_FILE}`);
    console.error('[FATAL] The server is likely already running. Aborting startup.');
    process.exit(1);
}
...
try {
    ...
    console.log(`[OSCE Server] Lock file created with PID: ${process.pid}`);
} catch (err) {
    console.error('[FATAL] Could not write lock file:', err);
    process.exit(1);
}
*/
initializePassport();
...
app.post('/api/tts', async (req, res) => {
    try {
        ...
        // Map Gender to Voice ID (Simple logic for now)
...
app.post('/api/generate-case', async (req, res) => {
    try {
        // Originally "generate-case", now repurposing for "Surprise Me" (Random Fetch)
...
app.post('/api/assess-ddx', async (req, res) => {
    try {
        const result = await evaluationService.assessDifferential(req.body.caseId, req.body.submittedDDx, req.body.stage);
        res.json(result);
    } catch (error: any) {
        res.status(500).json({ error: error.message });
    }
...
app.post('/api/evaluate', async (req, res) => {
    try {
        const result = await evaluationService.evaluateSession(req.body.sessionId, req.body.transcript);
        res.json(result);
    } catch (error: any) {
        res.status(500).json({ error: error.message });
    }
